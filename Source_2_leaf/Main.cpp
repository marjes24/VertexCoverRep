//Original Code plus degree cover 1 Optimization when largest degree is one
...
vector<int> DFSCover(list<list<int> > graph,int edge_count){
	...
	while(!node_stack.empty()){
		...
		if(tmp->graph.size()!=0 && !(tmp->cover_Vertex.size()>curr_min_cover.size()) ){ //backtrack if no more nodes or if the node cover gest to big
			...
			} else if(largest_count == 1){ //largest degree is one so just complete the vector cover here don't branch out
				for(list<list<int> >::iterator it = tmp_graph.begin(); it != tmp_graph.end(); it++){
					list<int>::iterator it_v = (*it).begin();
						if( (++it_v) != (*it).end()){ // if vertex does have edge
							if(!inVector(tmp->cover_Vertex, (*it_v)))
								tmp->cover_Vertex.push_back((*it_v));
						}
				}
